...
public class CallbackSenderI implements CallbackSender {
	...
	@Override
	public void registerClient(String hostname, CallbackReceiverPrx proxy, Current current) {
		CallbackReceiverPrx prx = clients.putIfAbsent(hostname, proxy);
		if (prx == null)
	...
	private String broadCastMessage(String message, String sender) {
		...
		for (CallbackReceiverPrx client : clients.values()) {
			// Enviamos el mensaje broadcast
			client.receiveMessage("Broadcast from [" + sender + "]: " + msg);
		}
		return "Broadcasting message to all clients";
	}

	@Override
	public void removeClient(String hostname, Current current) {
